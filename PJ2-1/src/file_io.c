...
FileIOResult open_file(FileInfo *file, const char *filename) {
    ...
    if (file->length & PAGE_MASK) {
        //  ensure the length of file be the int times of pagesize
        close_file(file);
        return INVALID_LEN;
    }
...
// read in page where address is addr
FileIOResult read_page(Page *page, const FileInfo *file, off_t addr) {
    if (addr & PAGE_MASK) {
        // addr must be integer times of pagesize
        return INVALID_ADDR;
    }
...
// write out page to where address is addr
